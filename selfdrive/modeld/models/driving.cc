...
void model_publish(PubMaster &pm, uint32_t vipc_frame_id, uint32_t frame_id,
  ...
  fill_lead(lead, &net_outputs.lead[mdn_max_idx*(LEAD_MHP_GROUP_SIZE)], sigmoid(net_outputs.lead_prob[t_offset]));
  ...
  fill_lead(lead_future, &net_outputs.lead[mdn_max_idx*(LEAD_MHP_GROUP_SIZE)], sigmoid(net_outputs.lead_prob[t_offset]));
